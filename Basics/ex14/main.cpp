...
float testSudoku()
{
	...
	for (int i = 0; i < N_PROVES; i++)
	{
		...
		if (fitxerTest.is_open())
		{
			...
			cout << "Comment :=>>" << endl;
			cout << "Comment :=>> Resultat final esperat de la partida: "; mostraFinal(finalEsperat[i]); cout << endl;
			cout << "Comment :=>> ----------" << endl;
			cout << "Comment :=>> Resultat final obtingut de la partida: "; mostraFinal(final); cout << endl;
			if (finalEsperat[i] == final)
				cout << "Comment :=>> CORRECTE" << endl;
			else
			{
				cout << "Comment :=>> ERROR" << endl;
